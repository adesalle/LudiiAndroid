 ...
 *
 * @author Eric Piette
 * @remarks This ludeme is used for games with local state values associated
 * with pieces.
 */
public final class State extends BaseIntFunction {
    ...
    public int eval(final Context context) {
        ...
        if (context.game().isStacking() && containerId == 0) {
            ...
            if (level == null)
                return state.state(loc, type);

            return state.state(loc, level.eval(context), type);
        }
        final ContainerState cs = context.state().containerStates()[containerId];

        return cs.state(loc, type);
    }
    ...
    public long gameFlags(final Game game) {
        long stateFlag = locn.gameFlags(game) | GameType.SiteState;
