 ...
 */
public class SingleMustNotWhatEdge extends AtomicProposition {
     ...
     * @param chunkSize
     */
    public SingleMustNotWhatEdge(final int mustNotWhatSite, final int mustNotWhatValue, final int chunkSize) {
    ...
    public boolean matches(final State state) {
        return !state.containerStates()[0].matchesWhatEdge(wordIdx, mask, matchingWord);
    }
    ...
    public boolean equals(final Object obj) {
        ...
        if (!(obj instanceof SingleMustNotWhatEdge))
            return false;

        final SingleMustNotWhatEdge other = (SingleMustNotWhatEdge) obj;
        return (mask == other.mask && matchingWord == other.matchingWord && wordIdx == other.wordIdx);
    }

    @Override
    public String toString() {
        return "[Edge " + site + " must NOT contain " + value + "]";
    }
