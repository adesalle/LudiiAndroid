...
public class SingleMustNotWhoCell extends AtomicProposition {
     ...
     * @param chunkSize
     */
    public SingleMustNotWhoCell(final int mustNotWhoSite, final int mustNotWhoValue, final int chunkSize) {
    ...
    public boolean matches(final State state) {
        assert
                (
                        state.containerStates()[0].matchesWhoCell(wordIdx, mask, matchingWord)
                                ==
                                (state.containerStates()[0].whoCell(site) == value)
                );
    ...
    public boolean equals(final Object obj) {
        ...
        final SingleMustNotWhoCell other = (SingleMustNotWhoCell) obj;
        return (mask == other.mask && matchingWord == other.matchingWord && wordIdx == other.wordIdx);
    }

    @Override
    public String toString() {
        return "[Cell " + site + " must NOT be owned by Player " + value + "]";
    }
