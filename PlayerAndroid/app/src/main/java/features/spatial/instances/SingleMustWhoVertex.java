 ...
 */
public class SingleMustWhoVertex extends AtomicProposition {
     ...
     * @param chunkSize
     */
    public SingleMustWhoVertex(final int mustWhoSite, final int mustWhoValue, final int chunkSize) {
    ...
    public boolean matches(final State state) {
        assert
                (
                        state.containerStates()[0].matchesWhoVertex(wordIdx, mask, matchingWord)
                                ==
                                (state.containerStates()[0].whoVertex(site) == value)
                );
    ...
    public boolean equals(final Object obj) {
        ...
        final SingleMustWhoVertex other = (SingleMustWhoVertex) obj;
        return (mask == other.mask && matchingWord == other.matchingWord && wordIdx == other.wordIdx);
    }

    @Override
    public String toString() {
        return "[Vertex " + site + " must be owned by Player " + value + "]";
    }
