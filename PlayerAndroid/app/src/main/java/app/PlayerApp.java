...
public abstract class PlayerApp implements PlayerInterface, PlatformGraphics {
    ...
    public abstract Rect[] playerSwatchList();

    public abstract Rect[] playerNameList();

    public abstract boolean[] playerSwatchHover();

    public abstract boolean[] playerNameHover();

    public abstract void repaint(Rect rect);
    ...
    private void drawCountValue(final Context context, final Graphics2D g2d, final ImageInfo imageInfo, final String count, final int numberCountsDrawn, final int totalCountsDrawn) {
        ...
        int fontSize = Math.min(imageInfo.imageSize() / 3, 20);
        g2d.setFont(typeFace, fontSize);
        ...
        if (imageInfo.containerIndex() > 0) {
            ...
            final Rectangle2D countRect = g2d.getStringBounds("x" + count);
            ...
            final Rectangle2D countRect = g2d.getStringBounds(count);
