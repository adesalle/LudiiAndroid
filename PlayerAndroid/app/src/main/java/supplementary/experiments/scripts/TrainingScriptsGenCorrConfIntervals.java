 ...
 */
public class TrainingScriptsGenCorrConfIntervals {
     ...
     */
    private TrainingScriptsGenCorrConfIntervals() {
        // Do nothing
    }
    ...
    private static void generateScripts(final CommandLineArgParse argParse) {
        ...
        while (processIdx < processDataList.size()) {
            ...
            try (final PrintWriter writer = new UnixPrintWriter(new File(scriptsDir + jobScriptFilename), "UTF-8")) {
                ...
                writer.println("#SBATCH -o /home/" + userName + "/TrainFeaturesCorrConfIntervals/Out/Out_%J.out");
                writer.println("#SBATCH -e /home/" + userName + "/TrainFeaturesCorrConfIntervals/Out/Err_%J.err");
                ...
                while (numJobProcesses < PROCESSES_PER_JOB && processIdx < processDataList.size()) {
                    ...
                    String javaCall = StringRoutines.join
                            (
                                    ...
                                    "-jar",
                                    StringRoutines.quote("/home/" + userName + "/TrainFeaturesCorrConfIntervals/Ludii.jar"),
                                    "--expert-iteration",
                                    "--game",
                                    ...
                                    StringRoutines.quote
                                            (
                                                    "/home/" +
                                                            userName +
                                                            "/TrainFeaturesCorrConfIntervals/Out/" +
                                                            StringRoutines.cleanGameName(processData.gameName.replaceAll(Pattern.quote(".lud"), "")) + (processData.noConfInterval ? "_Without" : "_With") + "/"
                                            ),
                                    "--no-logging",
                                    "--max-wall-time",
                    ...
                    if (processData.noConfInterval)
                        javaCall += " --critical-value-corr-conf 0";

                    javaCall += " " + StringRoutines.join
                            (
                                    " ",
                                    ">",
                                    "/home/" + userName + "/TrainFeaturesCorrConfIntervals/Out/Out_${SLURM_JOB_ID}_" + numJobProcesses + ".out",
    ...
    private static class ProcessData {
        ...
        public final boolean noConfInterval;

        /**
         * Constructor
         *
         * @param gameName
         * @param noConfInterval
         */
        public ProcessData(final String gameName, final boolean noConfInterval) {
            this.gameName = gameName;
            this.noConfInterval = noConfInterval;
        }
    }
