...
/**
 * Script to generate scripts for heuristic optimisation runs for various
 * Ludus Latrunculorum rulesets.
 *
 * @author Dennis Soemers
 */
public class OptimLudusHeuristicsScriptsGen {
    /**
     ...
     */
    private OptimLudusHeuristicsScriptsGen() {
        // Do nothing
    }
    ...
    private static void generateScripts(final CommandLineArgParse argParse) {
        ...
        for (final String[] gameRulesetArray : GAMES_RULESETS) {
            ...
            final String jobScriptFilename = "EvolOptimHeuristics" + filepathsGameName + filepathsRulesetName + ".sh";
            ...
            try (final PrintWriter writer = new UnixPrintWriter(new File(scriptsDir + jobScriptFilename), "UTF-8")) {
                ...
                writer.println("#SBATCH -J EvolOptimHeuristics_" + filepathsGameName + filepathsRulesetName);
                writer.println("#SBATCH -o /work/" + userName + "/EvolOptimHeuristics/Out"
                        + filepathsGameName + filepathsRulesetName + "_%J.out");
                writer.println("#SBATCH -e /work/" + userName + "/EvolOptimHeuristics/Err"
                ...
                final String javaCall = StringRoutines.join
                        (
                                ...
                                "-jar",
                                StringRoutines.quote("/home/" + userName + "/EvolOptimHeuristics/Ludii.jar"),
