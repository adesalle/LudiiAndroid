 ...
 * many games, requesting only a quarter of a node.
 *
...
public class ComputeConceptsScriptsGenLemaitre4QuarterNode {
    ...
    /**
     * Number of cores to use per game
     */
     ...
     */
    private static final int PROCESSES_PER_JOB = CORES_PER_JOB / CORES_PER_GAME;
    ...
    private static void generateScripts(final CommandLineArgParse argParse) {
        ...
        while (processIdx < processDataList.size()) {
            ...
            try (final PrintWriter writer = new UnixPrintWriter(new File(scriptsDir + jobScriptFilename), "UTF-8")) {
                ...
                writer.println("#SBATCH --cpus-per-task=" + (numProcessesThisJob * CORES_PER_GAME));
                writer.println("#SBATCH --mem=" + jobMemRequestGB + "G");        // 1 node, no MPI/OpenMP/etc

                totalRequestedCoreHours += (CORES_PER_JOB * (MAX_WALL_TIME / 60));
                        ...
                        (
                                StringRoutines.join
                                        (
                                                ...
                                                "--num-cores-total",
                                                String.valueOf(CORES_PER_JOB),
                                                "--num-threads-per-job",
                                                String.valueOf(CORES_PER_GAME),
                                                "--json-files"
                                        )
                        );

                // Put up to 12 jsons in this job
                int numJobProcesses = 0;
                while (numJobProcesses < 12 && processIdx < processDataList.size()) {
