...
/**
 * Method to generate cluster job scripts for playing lots of trials of a given
 * game until we crash, and then saving the trial right before we crashed.
 *
...
public class FindCrashingTrialScriptsGen {
    ...
    private static void generateScripts(final CommandLineArgParse argParse) {
        ...
        final int numJobs = argParse.getValueInt("--num-jobs");

        for (int jobID = 0; jobID < numJobs; ++jobID) {
            final String jobScriptFilename = "FindCrashingTrial_" + jobID + ".sh";
            ...
            try (final PrintWriter writer = new UnixPrintWriter(new File(scriptsDir + jobScriptFilename), "UTF-8")) {
                ...
                writer.println("#SBATCH -J FindCrashingTrial_" + jobID);
                writer.println("#SBATCH -o /work/" + userName + "/FindCrashingTrial/Out_"
                        + jobID + "_%J.out");
                writer.println("#SBATCH -e /work/" + userName + "/FindCrashingTrial/Err_"
                        + jobID + "_%J.err");
                ...
                @SuppressWarnings("unchecked") final String[] agentsStrings = ((List<String>) argParse.getValue("--agents")).toArray(new String[0]);
                for (int i = 0; i < agentsStrings.length; ++i) {
                    agentsStrings[i] = StringRoutines.quote(agentsStrings[i]);
                }
                ...
                final String javaCall = StringRoutines.join
                        (
                                ...
                                "-jar",
                                StringRoutines.quote("/home/" + userName + "/FindCrashingTrial/Ludii.jar"),
                                "--find-crashing-trial",
                                "--game",
                                StringRoutines.quote(argParse.getValueString("--game")),
                                "-n",
                                "" + argParse.getValueInt("--num-trials-per-job"),
                                "--game-length-cap 1000",
                                "--thinking-time " + argParse.getValueDouble("--thinking-time"),
                                "--depth-limit " + argParse.getValueInt("--depth-limit"),
                                "--iteration-limit " + argParse.getValueInt("--iteration-limit"),
                                "--out-trial-file",
                                ...
                                "--agents",
                                StringRoutines.join(" ", agentsStrings),
                                "--max-wall-time",
                                "" + Math.max((MAX_WALL_TIME), 60),
                                "--no-print-out"
                        );
    ...
    public static void main(final String[] args) {
        ...
        final CommandLineArgParse argParse =
                new CommandLineArgParse
                        (
                                true,
                                "Play many trials of a game, save trials right before crashes."
                        );
        ...
        argParse.addOption(new ArgOption()
                .withNames("--num-trials-per-job")
                .help("Number of trials to run per job.")
                .withNumVals(1)
        ...
        argParse.addOption(new ArgOption()
                .withNames("--num-jobs")
                .help("Number of (copies of) jobs to run.")
                .withNumVals(1)
                .withType(OptionTypes.Int)
                .withDefault(Integer.valueOf(100)));
        ...
        argParse.addOption(new ArgOption()
                .withNames("--agents")
                .help("Agents to use for playing")
