...
public class MiscLoading
{
	...
	public static void loadSVG(final PlayerApp app, final MainWindowDesktop view)
	{
		...
		//final String fileName = FileLoading.selectFile(AndroidApp.frame(), true, "/../Common/img/svg/", "SVG files (*.svg)", view, "svg");
		//if (fileName == null)
		//	return;
		...
		final int sz = (Math.min(AndroidApp.frame().getWidth()/2, AndroidApp.frame().getHeight()-40)) - 20;
		svgFrame.setSize((sz + 20) * 2, sz + 60);
		svgFrame.setLocationRelativeTo(AndroidApp.frame());
	...
	public static void loadTournamentFile(final PlayerApp app)
	{
		...
		final int fcReturnVal = AndroidApp.loadTournamentFileChooser().showOpenDialog(AndroidApp.frame());
		if (fcReturnVal == JFileChooser.APPROVE_OPTION)
		{
			final File file = AndroidApp.loadTournamentFileChooser().getSelectedFile();
