package playerAndroid.app.manualGeneration;
...
public class ManualGeneration
{
	 ...
	 */
	public static void manualGeneration(final AndroidApp app)
	{
		...
		if (includeHandMoves && app.manager().ref().context().game().requiresHand())
			AndroidApp.frame().setSize(800, 465);
		else
			AndroidApp.frame().setSize(410, 465);
	 ...
	 */
	private final static void generateSetupImage(final AndroidApp app)
	{
		GameUtil.resetGame(app, true);
		app.repaint();

		final Timer setupScreenshotTimer = new Timer();
		setupScreenshotTimer.schedule(new TimerTask()
		{
		    @Override
		    public void run()
		    {
		    	if (ScreenCapture.screenshotComplete() && ScreenCapture.gifAnimationComplete() && !AndroidApp.view().isPainting)
		    	{
	 ...
	 */
	private final static void generateMoveImages(final AndroidApp app, final List<MoveCompleteInformation> condensedMoveList)
	{
		final Timer moveScreenshotTimer = new Timer();
		moveScreenshotTimer.schedule(new TimerTask()
		{
		    ...
		    public void run()
		    {
		    	if (setupImageTimerComplete && ScreenCapture.screenshotComplete() && ScreenCapture.gifAnimationComplete() && !AndroidApp.view().isPainting)
		    	{
	    			condensedMoveIndex++;
	 ...
	 */
	private final static void generateEndImages(final AndroidApp app, final List<MoveCompleteInformation> endingMoveList)
	{
		...
		endScreenshotTimer.scheduleAtFixedRate(new TimerTask()
		{
		    ...
		    public void run()
		    {	
		    	if (generateMoveImagesTimerComplete && ScreenCapture.screenshotComplete() && ScreenCapture.gifAnimationComplete() && !AndroidApp.view().isPainting)
		    	{
	 ...
	 */
	private final static void generateWebsite(final AndroidApp app, final List<String> rankingStrings, final List<MoveCompleteInformation> condensedMoveList, final List<MoveCompleteInformation> endingMoveList)
	{
		final Referee ref = app.manager().ref();
		...
		generateWebsiteTimer.scheduleAtFixedRate(new TimerTask()
		{
		    ...
		    public void run()
		    {	
		    	if (generateEndImagesTimerComplete && !AndroidApp.view().isPainting && ScreenCapture.screenshotComplete() && ScreenCapture.gifAnimationComplete())
		    	{
		    		System.out.println("------------------------");
	 ...
	 */
	protected final static void takeMoveImage(final AndroidApp app, final MoveCompleteInformation moveInformation, final boolean endingMove)
	{
		...
		gifAnimationTimer.scheduleAtFixedRate(new TimerTask()
		{
		    ...
		    public void run()
		    {
		    	if (!AndroidApp.view().isPainting && ScreenCapture.screenshotComplete())
		    	{
		    		ScreenCapture.resetGifAnimationVariables();
	        		ScreenCapture.resetScreenshotVariables();
		...
		afterScreenShotTimer.scheduleAtFixedRate(new TimerTask()
		{
		    ...
		    public void run()
		    {
		    	if (!AndroidApp.view().isPainting && ScreenCapture.gifAnimationComplete())
		    	{
		    		ScreenCapture.resetScreenshotVariables();
