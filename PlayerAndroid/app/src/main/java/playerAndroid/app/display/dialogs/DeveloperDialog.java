...
import playerAndroid.app.display.dialogs.util.JComboCheckBox;
				...
				public void actionPerformed(final ActionEvent e)
				{
					EventQueue.invokeLater(new Runnable()
					{
						...
						public void run()
						{
							...
							{
								app.bridge().settingsVC().drawSideCells().put(((JCheckBox) directionLimitOptions.getItemAt(i)).getText().toString(), Boolean.valueOf(((JCheckBox) directionLimitOptions.getItemAt(i)).isSelected()));
							}
							app.repaint();
						}
					});
						...
						public void run()
						{
							for (int i = 0; i < directionLimitOptions.getItemCount(); i++)
							{
								app.bridge().settingsVC().drawSideVertices().put(
										((JCheckBox) directionLimitOptions.getItemAt(i)).getText().toString(),
			...
			private void update(final DocumentEvent documentEvent)
			{
				try
				{
					app.contextSnapshot().getContext(app).game()
							.setMaxMoveLimit(Integer.parseInt(textFieldMaximumNumberOfTurns.getText().toString()));
				}
				catch (final Exception e)
				{
					// not an integer;
