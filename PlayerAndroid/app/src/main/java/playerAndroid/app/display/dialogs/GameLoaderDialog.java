...
import androidUtils.awt.KeyboardFocusManager;
...
import androidUtils.swing.tree.DefaultTreeModel;
...
public class GameLoaderDialog
{
	...
	)
	{
		...
		filterField.setForeground(new ColorDrawable(Color.GRAY.toArgb()));
		...
		{  
			...
			{  
				...
				EventQueue.invokeLater(new Runnable()
				{
					...
					public void run()
					{
						final String currentSearchString = filterField.getTextContent();
			...
			public void setTextColour()
			{
				...
				{  
					filterField.setFont(gainFont);  
					filterField.setForeground(new ColorDrawable(Color.BLACK.toArgb()));
				} 
			}
		});  
		...
		contentPane.addHierarchyListener(new HierarchyListener()
		{
			...
			{
				final JDialog window = SwingUtilities.getRootJDialog(contentPane);
				if (window != null)
				{
                    if (!((Dialog) window).isResizable())
						((Dialog) window).setResizable(true);
					
					((Dialog) window).setLocationRelativeTo(AndroidApp.frame());
		...
		final KeyEventDispatcher keyDispatcher = e -> {
            if (e.getID() == KeyEvent.KEY_PRESSED)
            {
                ...
                )
                {
                    ...
                    {
                        oldSearchString = Objects.requireNonNull(filterField.getText()).toString();
	...
	private static class GameLoaderTree extends JTree
	{
		...
		public void updateTreeFilter(final JTextField filterField)
		{
			...
			String filterText = 
					Objects.requireNonNull(filterField.getText()).toString()
					.toLowerCase()
					.replaceAll(Pattern.quote("-"), "")
					.replaceAll(Pattern.quote(" "), "")
					.replaceAll(Pattern.quote("'"), "");
