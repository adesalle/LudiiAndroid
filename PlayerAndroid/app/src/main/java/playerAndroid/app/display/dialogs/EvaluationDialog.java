...
public class EvaluationDialog extends JDialog
{
	...
	public EvaluationDialog(final PlayerApp app)
	{
		...
		setBounds(100, 100, 780, AndroidApp.frame().getHeight());
		...
		final String[] comboBoxContents = AndroidGUIUtil.getAIDropdownStrings(app, false).toArray(new String[AndroidGUIUtil.getAIDropdownStrings(app, false).size()]);
		...
		okButton.addActionListener(new ActionListener()
		{
			...
			{	
				...
				switch(comboBoxAIAgents.getSelectedItem().toString())
				{
					...
					case "Custom": AIName = comboBoxAlgorithm.getSelectedItem().toString(); thinkTime = Double.valueOf(textFieldThinkTime.getTextContent()).doubleValue(); break;
				}
				...
				for (final Metric m : metrics)
				{
					...
					{
						((IdealDuration)m).setMinTurn(Double.valueOf(textFieldMinIdealTurns.getTextContent()).doubleValue());
						((IdealDuration)m).setMaxTurn(Double.valueOf(textFieldMaxIdealTurns.getTextContent()).doubleValue());
					}
					
					if (m instanceof SkillTrace)
					{
						((SkillTrace)m).setNumMatches(Integer.valueOf(textFieldNumMatches.getTextContent()).intValue());
						((SkillTrace)m).setNumTrialsPerMatch(Integer.valueOf(textFieldNumTrialsPerMatch.getTextContent()).intValue());
						((SkillTrace)m).setHardTimeLimit(Integer.valueOf(textFieldHardTimeLimit.getTextContent()).intValue());
						((SkillTrace)m).setOutputPath(txtcommonresoutput.getTextContent() + File.separatorChar);
					}
				}
		...
		skillTraceButton.addActionListener(new ActionListener()
		{
			...
			{	
				...
				for (int i = 0; i < allMetricSliders.size(); i++)
					weights.set(i, Double.valueOf(allMetricTextFields.get(i).getTextContent()));
			}
		});
		...
		for (int i = 0; i < metrics.size(); i++)
		{
			...
			allMetricSliders.get(metricIndex).addChangeListener(new ChangeListener()
			{
				...
				public void stateChanged(final ChangeEvent arg0)
				{
					...
					weights.set(metricIndex, Double.valueOf(allMetricTextFields.get(metricIndex).getTextContent()));
				}
			});
		}
