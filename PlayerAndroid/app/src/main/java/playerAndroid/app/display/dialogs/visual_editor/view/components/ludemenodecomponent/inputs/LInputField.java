...
import androidUtils.swing.Box;
import androidUtils.swing.DefaultFormatter;
import androidUtils.swing.JComboBox;
import androidUtils.swing.JComponent;
import androidUtils.swing.JFormattedTextField;
import androidUtils.swing.JLabel;
import androidUtils.swing.JOptionPane;
import androidUtils.swing.JSpinner;
...
public class LInputField extends JComponent
{
    ...
    private void loadButtons()
    {
        ...
        optionalLabel.setForeground(new ColorDrawable(DesignPalette.FONT_LUDEME_INPUTS_COLOR().toArgb()));
        optionalLabel.setText("(optional)");

        terminalOptionalLabel.setFont(DesignPalette.LUDEME_INPUT_FONT);

        if(terminalOptionalLabel.getMouseListeners().isEmpty())
    ...
    private final ActionListener expandButtonListener = new ActionListener() {
        @Override
    ...
    private final ActionListener addItemButtonListener = new ActionListener() {
        @Override
        public void actionPerformed(ActionEvent e) {
            addCollectionItem();
        }
    };
    private final ActionListener removeItemButtonListener = new ActionListener() {
        @Override
        public void actionPerformed(ActionEvent e) {
            removeCollectionItem();
        }
    };
    private final ActionListener choiceButtonListener = new ActionListener() {
    @Override
    public void actionPerformed(ActionEvent e) {
    ...
    private void construct(NodeArgument nodeArgument)
    {
        ...
        label.setForeground(new ColorDrawable(DesignPalette.FONT_LUDEME_INPUTS_COLOR().toArgb()));
    ...
    private void constructTerminal(NodeArgument nodeArgument, boolean removable)
    {
        ...
        if(nodeArgument.choice())
        {
            ...
            ViewComponent component = (ViewComponent) fieldComponent;
            component.setPreferredSize(new Dimension(component.getPreferredSize().width-choiceButton.getPreferredSize().width, component.getPreferredSize().height));
        ...
        if(removable)
        {
            ...
            {
                ((View) fieldComponent).setEnabled(false);
    ...
    private void initializeFieldComponent()
    {
        ...
        if(fieldComponent instanceof JTextField)
        {
            ...
            for(KeyListener listener : (ArrayList<KeyListener>)field.getKeyListeners().clone())
            {
                field.removeKeyListener(listener);
            }
            field.addKeyListener(userInputListener_keyListener);
        }
        else if(fieldComponent instanceof JSpinner)
        {
            for(ChangeListener listener : (ArrayList<ChangeListener>)((JSpinner)fieldComponent).getChangeListeners().clone())
            {
        ...
        {
            JComponent component = (JComponent) fieldComponent;
            for(PropertyChangeListener listener : (ArrayList<PropertyChangeListener>)component.getPropertyChangeListeners().clone())
            {
                component.removePropertyChangeListener(listener);
            }
            component.addPropertyChangeListener(userInputListener_propertyChange);
        }
    ...
    private void loadFieldComponentColours()
    {
        ...
        if(fieldComponent instanceof JTextField)
        {
            fieldComponent.setBackground(new ColorDrawable(DesignPalette.INPUT_FIELD_BACKGROUND().toArgb()));
            fieldComponent.setForeground(new ColorDrawable(DesignPalette.INPUT_FIELD_FOREGROUND().toArgb()));
            ((JTextField)fieldComponent).setBorder(b);
        }
        else if(fieldComponent instanceof JSpinner)
        {

            ((JSpinner) fieldComponent).getEditor().getComponent(0).setBackground(new ColorDrawable(DesignPalette.INPUT_FIELD_BACKGROUND().toArgb()));
            ((JSpinner) fieldComponent).getEditor().getComponent(0).setForeground(new ColorDrawable(DesignPalette.INPUT_FIELD_FOREGROUND().toArgb()));
        }
        else if(fieldComponent instanceof JComboBox)
        {
            ((JComboBox<?>) fieldComponent).getEditor().getEditorComponent().setBackground(new ColorDrawable(DesignPalette.INPUT_FIELD_BACKGROUND().toArgb()));
            ((JComboBox<?>) fieldComponent).getEditor().getEditorComponent().setForeground(new ColorDrawable(DesignPalette.INPUT_FIELD_FOREGROUND().toArgb()));
        }
        else
        {
            fieldComponent.setBackground(new ColorDrawable(DesignPalette.INPUT_FIELD_BACKGROUND().toArgb()));
    ...
    private void adjustFieldComponentSize(View otherComponent)
    {

        if (fieldComponent instanceof ViewComponent)
        {
            ((ViewComponent) fieldComponent).setPreferredSize(new Dimension(((ViewComponent) fieldComponent).getPreferredSize().width - ((ViewComponent) otherComponent).getPreferredSize().width, ((ViewComponent) fieldComponent).getPreferredSize().height));
            ((ViewComponent) fieldComponent).setSize(((ViewComponent) fieldComponent).getPreferredSize());
        }
     ...
     */
    private View generateTerminalComponent(NodeArgument nodeArgument)
    {
        ...
        {
            JSpinner spinner = new JSpinner(new JSpinner.SpinnerNumberModel(1, 1, Integer.MAX_VALUE, 1));
        ...
        {
            JSpinner spinner = new JSpinner(new JSpinner.SpinnerNumberModel(1, 0.01, Float.MAX_VALUE, 0.1));
            ((DefaultFormatter) ((JFormattedTextField) spinner.getEditor().getComponent(0)).getFormatter()).setCommitsOnValidEdit(true);
            return spinner;
        }
    ...
    public void activateHybrid(boolean activate)
    {
        ...
        fieldComponent.setVisibility(activate ? VISIBLE: INVISIBLE);
        if(activate)
        {
    ...
    final ActionListener userInputListener_dropdown = new ActionListener() {
        @Override
    ...
    public void setUserInput(Object input)
    {
        ...
        else
        {
            ...
            if (fieldComponent instanceof JComboBox<?> && input instanceof Symbol)
            {
                ...
                JComboBox<Symbol> comboBox = (JComboBox<Symbol>) fieldComponent;
                comboBox.setSelectedItem((Symbol) input);
            }
        }
    }
    ...
    private void updateTerminalComponentSize()
    {
        if(fieldComponent instanceof ViewComponent)
        {
            ...
            component.setFont(DesignPalette.LUDEME_INPUT_FONT);
            ...
            component.setSize(new Dimension(preferredWidth, component.getPreferredSize().height));
            if(component instanceof JComboBox<?>)
            {
                component.setPreferredSize(new Dimension(preferredWidth, DesignPalette.TERMINAL_INPUT_HEIGHT));
                component.setSize(new Dimension(preferredWidth, DesignPalette.TERMINAL_INPUT_HEIGHT));
            }
        }
    ...
    private Dimension buttonSize()
    {
        ...
        {
            if (fieldComponent instanceof ViewComponent)
                return new Dimension((int) (((ViewComponent) fieldComponent).getPreferredSize().height*buttonWidthPercentage), (int) (((ViewComponent) fieldComponent).getPreferredSize().height*buttonWidthPercentage));
            return null;
        }
    ...
    protected void paintComponent(Graphics g)
    {
        ...
        if(label.getForeground() != new ColorDrawable(DesignPalette.FONT_LUDEME_INPUTS_COLOR().toArgb()))
        {
            label.setForeground(new ColorDrawable(DesignPalette.FONT_LUDEME_INPUTS_COLOR().toArgb()));
            optionalLabel.setForeground(new ColorDrawable(DesignPalette.FONT_LUDEME_INPUTS_COLOR().toArgb()));
            terminalOptionalLabel.setForeground(new ColorDrawable(DesignPalette.FONT_LUDEME_INPUTS_COLOR().toArgb()));
        }
        ...
        if(fieldComponent != null && fieldComponent.getBackground() != new ColorDrawable(DesignPalette.INPUT_FIELD_BACKGROUND().toArgb()) && !(fieldComponent instanceof JComboBox)) // JComboBox background does not work
            loadFieldComponentColours();
