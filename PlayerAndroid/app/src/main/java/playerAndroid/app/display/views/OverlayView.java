package playerAndroid.app.display.views;
...
public final class OverlayView extends View
{
    ...
    {
        super(app);
        AndroidApp.frame().add(englishDescriptionField);
    }
    ...
    public void paint(final Graphics2D g2d)
    {
        ...
        final ToolView toolview = AndroidApp.view().toolPanel();
...
//					g2d.drawString("2. " + EnglishSwedishTranslations.DRAGPIECES.toString(), 50, 375);
        ...
        else
        {
            ...
            (
                    !app.bridge().settingsVC().selectedFromLocation().equals(new FullLocation(Constants.UNDEFINED))
                            ...
                            &&
                            AndroidApp.view().getMousePosition() != null
            )
                drawDraggedPiece(g2d, app.bridge().settingsVC().selectedFromLocation(), (int) AndroidApp.view().getMousePosition().getX(), (int) AndroidApp.view().getMousePosition().getY());
        }
    ...
    protected static void drawLoginDisc(final PlayerApp app, final Graphics2D g2d)
    {
        ...
        ImageProcessing.ballImage(g2d, AndroidApp.view().getWidth()-r*2-r, r, r, markerColour);
    }
    ...
    private void calculateFont()
    {
        ...
        final int fontMultiplier  = (int) (app.bridge().getContainerStyle(context.board().index()).cellRadius() * 2 * AndroidApp.view().getBoardPanel().boardSize());
    ...
    private void drawExtraGameInformation(final Graphics2D g2d, final Context context)
    {
        ...
        else if (AndroidApp.view().temporaryMessage().length() > 0)
        {
            drawStringBelowBoard(g2d, AndroidApp.view().temporaryMessage(), 0.98);
        }
...
//		}
    ...
    {
        final int pixels = AndroidApp.view().getBoardPanel().placement().width;
