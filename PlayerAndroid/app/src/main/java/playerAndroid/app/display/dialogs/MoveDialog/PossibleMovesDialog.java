...
public class PossibleMovesDialog extends MoveDialog
{
	...
	public static void createAndShowGUI(final PlayerApp app, final Context context, final FastArrayList<Move> validMoves, final boolean centerOnBoard)
	{
		...
		try
		{
			...
			if (centerOnBoard)
				drawPosn = new Point(
						(int)(AndroidApp.frame().getX() + AndroidApp.view().getBoardPanel().placement().getCenterX() - dialog.getWidth() / 2),
						(int)(AndroidApp.frame().getY() + AndroidApp.view().getBoardPanel().placement().getCenterY() - dialog.getHeight() / 2) + menuBarHeight);
