...
import androidUtils.swing.text.SimpleAttributeSet;
import androidUtils.swing.text.StyleConstants;
import androidUtils.swing.text.StyleContext;
...
public class EditorDialog extends JDialog
{
		...
		{			
			...
			dialog.addWindowListener(new WindowAdapter()
			{
				...
				public void windowClosed(final WindowEvent e)
				{
					...
					AndroidApp.view().invalidate();								// WAS: setSize(AndroidApp.view().getSize()); let's see if anybody screams!
					app.repaint();
	...
	{
		super(null, androidUtils.awt.Dialog.ModalityType.DOCUMENT_MODAL);
...
//		addMouseMotionListener(textArea);
	...
	private static JTextPane createTextPane()
	{
		...
		if (styledDoc instanceof Document)
		{
			final Document doc = (Document)styledDoc;
		...
		textArea.addMouseListener(new MouseAdapter()
		{
			...
			public void mouseClicked(final MouseEvent e)
			{
				...
				else if (e.getClickCount()==2)
				{
					...
					final TokenRange range = Parser.tokenScope(String.valueOf(textArea.getText()), caretPos, true, SelectionType.SELECTION);
					if (range != null) 
					{
...
//			@Override public void mouseExited(MouseEvent e)
	...
	private static JButton addCancelButton(final JPanel buttonPane)
	{
		final ActionListener listener = new ActionListener()
		{
			...
			public void actionPerformed(final ActionEvent e)
			{
				final View component = (View) e.getSource();
				final JDialog dialog1 = SwingUtilities.getRootJDialog(component);
				dialog1.dispose();
			}
		};
		...
		final ActionListener listener = new ActionListener()
		{
			...
			public void actionPerformed(final ActionEvent e)
			{
				...
				if (fcReturnVal == JFileChooser.APPROVE_OPTION)
				{
					...
					final File file = AndroidApp.gameFileChooser().getSelectedFile();
					try
					{
	...
	{
		final int fcReturnVal = AndroidApp.saveGameFileChooser().showSaveDialog(AndroidApp.frame());

		if (fcReturnVal == JFileChooser.APPROVE_OPTION)
		{
			File file = AndroidApp.saveGameFileChooser().getSelectedFile();
	...
	final String getText() { return String.valueOf(textArea.getText()); }
	...
	final void applyBackspace(final PlayerApp app)
	{
		...
		setText(app, String.valueOf(textArea.getText()));
	}
	...
	final void replaceTokenScopeWith(final PlayerApp app, final String substitution, final boolean isPartial)
	{
			...
			final TokenRange range = Parser.tokenScope(String.valueOf(textArea.getText()), caretPos, isPartial, isPartial ? SelectionType.TYPING : SelectionType.CONTEXT);
			if (range == null) 
			{
	...
	{
		if (String.valueOf(textArea.getText()).isEmpty()) return "";
