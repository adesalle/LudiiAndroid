 ...
 */
public class MainMenuFunctionsOld extends JMenuBar
{
    ...
    public static void checkActionsPerformed(final AndroidApp app, final ActionEvent e)
    {
        ...
        {
            // AndroidApp.frame().setSize(462, 462); Eric Resolution
            AndroidApp.frame().setSize(464, 464);
...
//			System.out.println("Num Forced Passes: " + app.manager().ref().context().trial().numForcedPasses());
                    ...
                    EventQueue.invokeLater(() ->
                    {
                        EventQueue.invokeLater(() ->
                        {
                            EventQueue.invokeLater(() ->
                            {
                                ...
                                AndroidApp.frame().setContentPane(AndroidApp.view());
        ...
        {
            AndroidApp.view().toolPanel().buttons.get(ToolView.BACK_BUTTON_INDEX).press();
        }
        else if (source.getText().equals("Next Move"))
        {
            AndroidApp.view().toolPanel().buttons.get(ToolView.FORWARD_BUTTON_INDEX).press();
        }
        else if (source.getText().equals("Go To Start"))
        {
            AndroidApp.view().toolPanel().buttons.get(ToolView.START_BUTTON_INDEX).press();
        }
        else if (source.getText().equals("Go To End"))
        {
            AndroidApp.view().toolPanel().buttons.get(ToolView.END_BUTTON_INDEX).press();
        }
        else if (source.getText().equals("Pass"))
        {
            AndroidApp.view().toolPanel().buttons.get(ToolView.PASS_BUTTON_INDEX).press();
        }
        ...
        else if (source.getText().startsWith("Jump to Move"))
        {
            try
            {
                final int moveToJumpTo = Integer.parseInt(JOptionPane.showInputDialog(AndroidApp.frame(), "Which move to jump to?"));
...
//			String tempFilePath = AndroidApp.lastSelectedJsonPath();
...
//
//			final int choice = fileChooser.showOpenDialog(AndroidApp.frame());
...
//			}
        ...
        {
            final String heuristicStr = JOptionPane.showInputDialog(AndroidApp.frame(), "Enter heuristic.");

            if (app.manager().settingsNetwork().getActiveGameId() == 0)
            {
                final Heuristics heuristics =
                        (Heuristics) Compiler.compileObject
                                (
        ...
        {
            final String featuresStr = JOptionPane.showInputDialog(AndroidApp.frame(), "Enter features.");

            if (app.manager().settingsNetwork().getActiveGameId() == 0)
            {
                final Features features =
                        (Features) Compiler.compileObject
                                (
                                        featuresStr,
...
//		}
        ...
        {
            MiscLoading.loadSVG(app, AndroidApp.view());
        }
        else if (source.getText().equals("View SVG"))
        {
            SVGViewerDialog.showDialog(app, AndroidApp.frame(), SVGLoader.listSVGs());
        }
    ...
    {
        MainMenuFunctionsOld.startVisualEditor = startVisualEditor;
    }
