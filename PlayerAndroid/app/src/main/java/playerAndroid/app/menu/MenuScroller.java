...
public class MenuScroller 
{
  ...
  public void keepVisible(JMenuItem item) {
    if (item == null) {
      ...
      final int index = menu.getComponentIndex(item);
  ...
  private class MenuScrollTimer extends Timer implements Serializable {
  ...
  private static enum MenuIcon implements Icon {
    ...
    public void paintIcon(View c, Graphics g, int x, int y) {
      final Dimension size = new Dimension(getIconWidth(), getIconHeight());
      final Graphics g2 = (Graphics) g.create(size.width / 2 - 5, size.height / 2 - 5, 10, 10);
