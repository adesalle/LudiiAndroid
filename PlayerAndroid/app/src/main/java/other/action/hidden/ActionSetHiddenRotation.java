...
/**
 * Sets the rotation hidden information to a graph element type at a specific
 * level for a specific player.
 *
 * @author Eric.Piette
 */
public final class ActionSetHiddenRotation extends BaseAction {
     ...
     */
    public ActionSetHiddenRotation(final int who, final SiteType type, final int to, final int level,
                                   final boolean value) {
        this.type = type;
        this.to = to;
        this.who = who;
    ...
    /**
     * Reconstructs an ActionSetHiddenRotation object from a detailed String
     * (generated using toDetailedString())
     *
     * @param detailedString
     */
    public ActionSetHiddenRotation(final String detailedString) {
        assert (detailedString.startsWith("[SetHiddenRotation:"));
    ...
    public Action apply(final Context context, final boolean store) {
        ...
        if (!alreadyApplied) {
            ...
            previousValue = cs.isHiddenRotation(who, to, level, type);
            previousType = type;
            alreadyApplied = true;
        }

        context.containerState(context.containerId()[to]).setHiddenRotation(context.state(), who, to, level, type, value);
        return this;
    }
    ...
    public Action undo(final Context context, boolean discard) {
        context.containerState(context.containerId()[to]).setHiddenRotation(context.state(), who, to, level, previousType, previousValue);
        return this;
    }
    ...
    public boolean equals(final Object obj) {
        ...
        if (!(obj instanceof ActionSetHiddenRotation))
            return false;

        final ActionSetHiddenRotation other = (ActionSetHiddenRotation) obj;
